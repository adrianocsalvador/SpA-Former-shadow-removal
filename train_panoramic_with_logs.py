...
from eval import test, test_sliding


def setup_memory_optimization():
    """Configura otimizações de memória"""
...
class TrainingLogger:
    ...
    def __init__(self, output_dir):
        ...
        with open(self.log_file, 'w') as f:
            f.write("=== TREINAMENTO SPA-FORMER ===\n")
    ...
    def log_epoch(self, epoch, total_epochs, loss_d, loss_g, loss_l1, loss_whitening, epoch_time, total_time):
        """Log de uma época"""
        # Adicionar métricas
    ...
    def plot_metrics(self):
        ...
        fig.suptitle('Métricas de Treinamento SPA-Former', fontsize=16)
        ...
        # Gráfico 4: Tempo total
        axes[1, 1].plot(self.metrics['epochs'], self.metrics['total_time'], 'y-', label='Tempo Total')
        axes[1, 1].set_title('Tempo Total de Treinamento')
...
def train_panoramic_with_logs(config):
    """Função principal de treinamento com logs detalhados"""
    
    # Setup inicial de memória
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        for iteration in range(total_batches):
            try:
                ...
                # Limpeza de memória a cada 20 iterações
                if iteration % 20 == 0 and getattr(config, 'enable_memory_cleanup', False):
                    clear_memory_aggressive()
                    
            except RuntimeError as e:
                if "out of memory" in str(e).lower():
                    print(f"⚠️ OOM na iteração {iteration}, limpando memória...")
                    clear_memory_aggressive()
                    continue
                else:
                    raise e

        # Calcular médias da época (evitar divisão por zero)
        if batch_count == 0:
            with open(logger.log_file, 'a') as f:
                f.write(f"Aviso: nenhuma iteração concluída na época {epoch} (provável OOM). Considere reduzir window_width/height ou gradient_accumulation_steps.\n")
            # Pular logging detalhado desta época e seguir para a próxima
            clear_memory_aggressive()
            continue
        ...
        logger.log_epoch(epoch, config.epoch, avg_loss_d, avg_loss_g, avg_loss_l1, 
                        avg_loss_whitening, epoch_time, total_time)
        ...
        if epoch % config.snapshot_interval == 0:
            # Validação sliding no GPU
            test_sliding(config, validation_data_loader, gen, criterionMSE, epoch)
...
if __name__ == '__main__':
    ...
    train_panoramic_with_logs(config)
