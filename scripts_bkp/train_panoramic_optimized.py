#!/usr/bin/env python3
"""
Treinamento otimizado para imagens panorâmicas com correção de branqueamento
"""
...
class PanoramicDataset:
    """Dataset otimizado para imagens panorâmicas com balanceamento"""
    ...
    def __init__(self, config, window_size=(256, 256), stride=(128, 128)):
        ...
        self.imlist = [f for f in os.listdir(train_a_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
    ...
    def _generate_windows_for_current_image(self):
        """Gera janelas para a imagem atual com balanceamento"""
        ...
        # Armazenar janelas com balanceamento
        self.current_windows = []
        for x, y in windows:
    ...
    def get_next_batch(self, batch_size):
        """Retorna próximo batch com balanceamento"""
        batch = []
...
def train_panoramic_optimized(config):
    """Função principal de treinamento otimizada"""
    gpu_manage(config)
    ...
    train_dataset = PanoramicDataset(config, window_size, stride)
    ...
    scaler = GradScaler() if hasattr(config, 'use_mixed_precision') and config.use_mixed_precision else None
...
if __name__ == '__main__':
    ...
    train_panoramic_optimized(config)
